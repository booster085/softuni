function solve(inputArr){
    var final = {},
        studentsArr = [],
        trainersArr = [], i, sortCriteria,
        roleType,
        newStudent, newTrainer;

    sortCriteria = inputArr[0].split('^')[0].trim();

    for (i = 1; i < inputArr.length; i += 1) {
        inputArr[i] = JSON.parse(inputArr[i]);
        roleType = inputArr[i].role;

        if (roleType === 'student') {
            studentsArr.push(inputArr[i])
        } else {
            trainersArr.push(inputArr[i]);
        }

    }
    switch (sortCriteria) {
        case 'level': studentsArr = studentsArr.sort(function(a, b){
            if (a.level !== b.level) {
            ...
            break;
        case 'name': studentsArr = studentsArr.sort(function(a, b){
    ...
    trainersArr = trainersArr.sort(function(a, b){
    ...
    for (i in studentsArr) {
        studentsArr[i] = createNewStudentObj(studentsArr[i]);
    }
    for (i in trainersArr) {
        trainersArr[i] = createNewTrainerObj(trainersArr[i]);
    }

    final.students = studentsArr;
    final.trainers = trainersArr;
    ...
    function createNewStudentObj(obj){
        var score = 0;
        for (i in obj.grades) {
            score += Number(obj.grades[i]);
        }
        score = score / obj.grades.length;
        newStudent = {
            id: obj.id,
            firstname: obj.firstname,
            lastname: obj.lastname,
            averageGrade: score.toFixed(2),
            certificate: obj.certificate
        };
        return newStudent;
    }
    function createNewTrainerObj(obj){
        newTrainer = {
            id: obj.id,
            firstname: obj.firstname,
            lastname: obj.lastname,
...
//ALWAYS USE localeCompare about non digits.
//ALWAYS USE localeCompare about non digits.
//ALWAYS USE localeCompare about non digits.
//ALWAYS USE localeCompare about non digits.
